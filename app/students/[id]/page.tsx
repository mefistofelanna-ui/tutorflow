...
import { Payment, readPayments, writePayments } from "../../payments/payment-data";
...
export default function StudentDetail(){
 ...
 useEffect(()=>{const timeout=window.setTimeout(()=>{const id=decodeURIComponent(window.location.pathname.split("/").filter(Boolean).pop()||"");setStudent(readStudents().find(item=>item.id===id)??null);setLessons(readLessons().filter(item=>item.studentId===id).sort((a,b)=>`${b.date} ${b.time}`.localeCompare(`${a.date} ${a.time}`)));setPayments(readPayments().filter(item=>item.studentId===id).sort((a,b)=>b.date.localeCompare(a.date)));setSeries(readSeries().find(item=>item.studentId===id)??null)},0);return()=>window.clearTimeout(timeout)},[]);
 const actualDays=useMemo(()=>series?`${series.weekdays.map(day=>labels[day]).join(", ")} · ${series.time}`:student?.days||"Плавающий график",[series,student]);
 const save=(value:Student,schedule?:StudentSchedule)=>{let nextLessons=readLessons(),nextSeries=readSeries();if(series&&schedule){if(!window.confirm("Применить новое расписание ко всем будущим занятиям?"))return;const changed:LessonSeries={...series,weekdays:schedule.weekdays,time:schedule.time,duration:schedule.duration,startDate:schedule.date,endDate:schedule.endDate};const future=buildSeriesLessons(changed).filter(item=>item.date>=today()),replaceIds=nextLessons.filter(item=>item.seriesId===series.id&&item.date>=today()&&(item.status==="scheduled"||item.status==="rescheduled")).map(item=>item.id),remaining=nextLessons.filter(item=>!replaceIds.includes(item.id)),conflicts=future.filter(item=>conflictsFor(item,remaining).length);if(conflicts.length){window.alert(`Найдены конфликты: ${conflicts.length}. Измените расписание.`);return}nextLessons=[...remaining,...future];nextSeries=nextSeries.map(item=>item.id===series.id?changed:item);value.days=`${changed.weekdays.map(day=>labels[day]).join(", ")} · ${changed.time}`;writeLessons(nextLessons);writeSeries(nextSeries);setSeries(changed);setLessons(nextLessons.filter(item=>item.studentId===value.id).sort((a,b)=>`${b.date} ${b.time}`.localeCompare(`${a.date} ${a.time}`)))}writeStudents(readStudents().map(item=>item.id===value.id?value:item));setStudent(value);setEditing(false);setToast("Данные ученика обновлены");window.setTimeout(()=>setToast(""),2300)};
 const archive=()=>{if(!student)return;writeStudents(readStudents().map(item=>item.id===student.id?{...item,archived:true}:item));writeLessons(readLessons().map(item=>item.studentId===student.id&&item.date>=today()&&(item.status==="scheduled"||item.status==="rescheduled")?{...item,status:"cancelled",seriesOverride:true}:item));window.location.href="/students"};
 const remove=()=>{if(!student)return;writeStudents(readStudents().filter(item=>item.id!==student.id));writeLessons(readLessons().filter(item=>item.studentId!==student.id));writePayments(readPayments().filter(item=>item.studentId!==student.id));writeSeries(readSeries().filter(item=>item.studentId!==student.id));window.location.href="/students"};
 ...
 const initialSchedule=series?{mode:"weekly" as const,date:series.startDate,time:series.time,duration:series.duration,weekdays:series.weekdays,endDate:series.endDate}:undefined;
