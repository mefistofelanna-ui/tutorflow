...
export default function StudentsShell({ children, toast }: { children: ReactNode; toast?: string }) {
  const [open,setOpen]=useState(false);
  const items=[["⌂","Главная","/"],["□","Расписание","#"],["♙","Ученики","/students"],["₽","Оплаты","#"],["↗","Статистика","#"]];
  return <main className="students-shell">
    <aside className={open?"students-sidebar open":"students-sidebar"}>
      ...
      <nav className="students-nav" aria-label="Основная навигация">{items.map(([icon,label,url])=><Link key={label} href={url} className={label==="Ученики"?"active":""} onClick={()=>setOpen(false)}><span>{icon}</span>{label}</Link>)}</nav>
