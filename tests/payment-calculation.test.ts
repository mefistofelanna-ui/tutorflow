...
test("legacy records are not recalculated even when their amount and count disagree", () => {
  const old = payment();
  const result = planPayment({ ...old, date: "2026-09-12" }, old, [student({ price: 1000, balance: 8 })]);
  assert.deepEqual(result.payment, { ...old, date: "2026-09-12" });
  assert.deepEqual(result.updates, []);
  assert.deepEqual(reversePayment({ balance: 8, moneyCredit: 200 }, old), { balance: 0, moneyCredit: 200 });
});

test("editing a legacy payment keeps its existing balance adjustment", () => {
  const old = payment();
  const result = planPayment({ ...old, lessonCount: 9 }, old, [student({ balance: 2, moneyCredit: 200 })]);
  assert.deepEqual(result.updates, [{ id: "student", balance: 3, moneyCredit: 200 }]);
});
